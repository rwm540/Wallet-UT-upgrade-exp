...
export const ApiPaymentModal: React.FC<ApiPaymentModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2.5 sm:p-4 bg-slate-900/70 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white rounded-3xl sm:rounded-[32px] shadow-2xl border border-slate-200 w-full max-w-3xl overflow-hidden flex flex-col max-h-[92vh] sm:max-h-[88vh]" dir="rtl">
        
        {/* Header */}
        <div className="bg-gradient-to-r from-slate-900 via-teal-950 to-emerald-950 px-4 sm:px-6 py-4 flex items-center justify-between text-white gap-3 border-b border-emerald-500/20 shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            <div className="p-2 sm:p-2.5 bg-emerald-500/20 rounded-2xl border border-emerald-400/30 text-emerald-300 shrink-0">
              <CreditCard className="w-5 h-5 sm:w-6 sm:h-6" />
            </div>
            <div className="min-w-0">
              <h2 className="text-sm sm:text-base md:text-lg font-black tracking-tight leading-tight truncate">سامانه API Payment و درگاه بانکی</h2>
              <p className="text-[10px] sm:text-xs text-slate-300 mt-0.5 truncate">مدیریت کلیدهای API، تست واریز و برداشت اتوماتیک</p>
            </div>
          </div>
          <button 
            onClick={onClose}
            className="p-2 sm:p-2.5 bg-white/10 hover:bg-white/20 rounded-2xl transition-all cursor-pointer text-slate-300 hover:text-white shrink-0 active:scale-95"
            title="بستن"
          >
        ...
        {/* Subheader Navigation Tabs - Scrollable with High Contrast & Clear Text */}
        <div className="bg-slate-100/90 px-3 sm:px-6 py-2.5 border-b border-slate-200 shrink-0 flex items-center gap-2 overflow-x-auto touch-pan-x scroll-smooth [scrollbar-width:thin] scrollbar-thin scrollbar-thumb-emerald-300 scrollbar-track-slate-100 min-w-0 w-full">
          <button
            onClick={() => setActiveTab('overview')}
            className={`px-4 py-2.5 rounded-2xl text-xs sm:text-sm font-black transition-all cursor-pointer whitespace-nowrap shrink-0 ${
              activeTab === 'overview' 
                ? 'bg-emerald-600 text-white shadow-md shadow-emerald-600/25 border-b-2 border-emerald-800' 
                : 'text-slate-700 hover:text-emerald-800 hover:bg-slate-200/80 bg-white/80 border border-slate-200/80'
            }`}
          >
            کلید API و تنظیمات
          </button>
        ...
        {/* Content Body */}
        <div className="p-4 sm:p-6 overflow-y-auto space-y-5 sm:space-y-6 flex-1 bg-slate-50/50" dir="rtl">
            ...
            <div className="space-y-4 sm:space-y-5">
              <div className="bg-white p-4 sm:p-5 rounded-3xl border border-slate-200 shadow-sm space-y-4">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="text-xs font-bold text-rose-600 hover:text-rose-700 flex items-center justify-center gap-1.5 bg-rose-50 hover:bg-rose-100 px-3 py-2 rounded-2xl transition-all cursor-pointer shrink-0 whitespace-nowrap w-full sm:w-auto"
                  >
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                  <input
                    ...
                    readOnly
                    value={apiKey}
                    className="flex-1 min-w-0 bg-slate-100 border border-slate-200 rounded-2xl px-3.5 py-3 sm:py-2.5 font-mono text-xs sm:text-sm text-slate-800 font-bold text-center sm:text-left focus:outline-none"
                    dir="ltr"
                  />
                  <button
                    onClick={handleCopyKey}
                    className="bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-3 sm:py-2.5 rounded-2xl font-bold text-xs sm:text-sm flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-md shadow-emerald-600/20 shrink-0 whitespace-nowrap"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-emerald-50/80 border border-emerald-200 p-4 rounded-3xl space-y-2">
                  ...
                  <code className="block bg-white border border-emerald-200/80 p-2.5 rounded-2xl text-xs font-mono text-emerald-900" dir="ltr">
                    POST /api/v1/payment/deposit
                  </code>
                  <p className="text-[11px] text-emerald-700 font-medium">برای ایجاد فاکتور و افزایش اتوماتیک موجودی UT کیف پول کاربر.</p>
                </div>
                ...
                <div className="bg-teal-50/80 border border-teal-200 p-4 rounded-3xl space-y-2">
                  ...
                  <code className="block bg-white border border-teal-200/80 p-2.5 rounded-2xl text-xs font-mono text-teal-900" dir="ltr">
                    POST /api/v1/payment/withdraw
                  </code>
                  <p className="text-[11px] text-teal-700 font-medium">برای ارسال درخواست تسویه و برداشت UT به حساب مقصد.</p>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="bg-white p-5 sm:p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
              ...
              <form onSubmit={handleRunDepositTest} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">مبلغ واریزی (UT)</label>
                    <input
                      type="number"
                      value={depositAmount}
                      onChange={(e) => setDepositAmount(e.target.value)}
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-2.5 text-sm font-mono font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                      dir="ltr"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">شناسه فاکتور (Reference ID)</label>
                    <input
                      type="text"
                      value={depositRefId}
                      onChange={(e) => setDepositRefId(e.target.value)}
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-2.5 text-sm font-mono text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                      dir="ltr"
                    />
                  </div>
                </div>
                ...
                <button
                  type="submit"
                  className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-2xl shadow-md shadow-emerald-600/20 transition-all cursor-pointer flex items-center justify-center gap-2 text-sm active:scale-98"
                >
          ...
          {activeTab === 'withdraw' && (
            <div className="bg-white p-5 sm:p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
              ...
              <form onSubmit={handleRunWithdrawTest} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">مبلغ برداشت (UT)</label>
                    <input
                      type="number"
                      value={withdrawAmount}
                      onChange={(e) => setWithdrawAmount(e.target.value)}
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-2.5 text-sm font-mono font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500"
                      dir="ltr"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">شماره حساب یا آدرس مقصد</label>
                    <input
                      type="text"
                      value={withdrawDestination}
                      onChange={(e) => setWithdrawDestination(e.target.value)}
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-2.5 text-sm font-mono text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500"
                      dir="ltr"
                    />
                  </div>
                </div>
                ...
                <button
                  type="submit"
                  className="w-full bg-teal-600 hover:bg-teal-500 text-white font-bold py-3 rounded-2xl shadow-md shadow-teal-600/20 transition-all cursor-pointer flex items-center justify-center gap-2 text-sm active:scale-98"
                >
                  <Play className="w-4 h-4" />
          ...
          {activeTab === 'docs' && (
            <div className="space-y-4 bg-white p-5 sm:p-6 rounded-3xl border border-slate-200 shadow-sm">
              ...
              <div className="bg-slate-900 text-slate-200 p-4 rounded-2xl font-mono text-xs overflow-x-auto" dir="ltr">
...
console.log(result);`}</pre>
        ...
        {/* Footer */}
        <div className="bg-slate-100 px-4 sm:px-6 py-3.5 border-t border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-3 text-center sm:text-right shrink-0">
          ...
          <button
            onClick={onClose}
            className="w-full sm:w-auto bg-slate-800 hover:bg-slate-700 text-white font-bold text-xs sm:text-sm px-6 py-2.5 rounded-2xl transition-all cursor-pointer shadow-md shrink-0 active:scale-95"
          >
            بستن پنجره
          </button>
        </div>
